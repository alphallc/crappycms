...
function _I.render()
	...
	cnt.title = "iBash.im без дна";

	local db_res = ngx.location.capture('/db/q/abyss?page='..(page or 0));
	if db_res.status ~= ngx.HTTP_OK then
		...
		q.total_quotes = json.decode(ngx.location.capture("/db/q/abysscount").body)[1].count;
