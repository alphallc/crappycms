...
function _I.render()
	...
	cnt.title = "iBash.im: мусорка";

	local db_res = ngx.location.capture('/db/q/trash?page='..(page or 0));
	if db_res.status ~= ngx.HTTP_OK then
		...
		q.total_quotes = json.decode(ngx.location.capture("/db/q/trashcount").body)[1].count;
